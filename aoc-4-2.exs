defmodule AOC4 do
  ...
  def filter_winning_board(boards) do
    Enum.filter(boards, fn board -> !is_winning_board(board) end)
  end
...
IO.puts(
  Enum.reduce(draws, boards, fn draw, acc ->
    if is_list(acc) do
      ...
      losing_boards = AOC4.filter_winning_board(marked_boards)

      if length(losing_boards) == 0 do
        AOC4.score(hd(marked_boards), draw)
      else
        losing_boards
      end
    else
