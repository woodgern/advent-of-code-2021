defmodule AOC12 do
  def find_paths(rules, current, visited, doubled) do
    if current == "end" do
      1
    else
      new_visited =
      ...
      Enum.reduce(rules[current] || [], 0, fn node, total ->
        if MapSet.member?(new_visited, node) do
          if !doubled and node != "start" and node != "end" do
            total + find_paths(rules, node, new_visited, true)
          else
            total
          end
        else
          total + find_paths(rules, node, new_visited, doubled)
        end
      end)
    end
  end
...
IO.puts(AOC12.find_paths(rules, "start", MapSet.new(), false))
